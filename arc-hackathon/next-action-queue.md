# KOLMarket Arc Next Action Queue

Updated: 2026-07-30

## Objective

Move KOLMarket from prepared materials to official Arc/Circle/Encode review surfaces without overstating completion.

## Current Evidence

- Public Arc package: https://github.com/dappweb/kolmarket/tree/main/arc-hackathon
- Public brief: https://github.com/dappweb/kolmarket/blob/main/arc-hackathon/public-brief.md
- Public submission draft: https://github.com/dappweb/kolmarket/blob/main/arc-hackathon/submission.md
- Public deck: https://github.com/dappweb/kolmarket/blob/main/arc-hackathon/deck.md
- X reply already posted: https://x.com/KOLMARKET/status/2082565862381507059
- Encode Hackathon registration/project creation observed.

## Not Yet Proven

- Arc official ecosystem listing
- Arc official repost
- Arc official builder spotlight
- Arc House post/comment published
- Discord builder message sent
- Circle grant application submitted
- Encode checkpoint submitted
- Encode final submission submitted

## Priority 1 - Encode Checkpoint

Use when the checkpoint form asks for repo/project/demo fields.

Project URL:

```text
https://kolmarket.ai
```

Repository or review package URL:

```text
https://github.com/dappweb/kolmarket/tree/main/arc-hackathon
```

Access instructions:

```text
Open https://kolmarket.ai and review the Arc package at https://github.com/dappweb/kolmarket/tree/main/arc-hackathon. The package includes public brief, submission draft, deck outline, evidence boundary, and status metadata. The current package documents the Arc Testnet pilot and review scope; fresh Arc transaction evidence and demo video are still being prepared.
```

Track:

```text
DeFi / programmable money
```

Secondary fit:

```text
Agentic economy / creator economy payments
```

Do not click final submit until the user confirms at action time.

## Priority 2 - Arc House Showcase

Title:

```text
KOLMarket: USDC-native creator growth markets on Arc
```

Body:

```text
KOLMarket is building USDC-native creator growth markets and campaign settlement infrastructure on Arc.

The Arc pilot focuses on aUSDC-denominated campaign budgets, creator/KOL attribution workflows, Growth Pool reward state, and creator-token market infrastructure.

Public review package:
https://github.com/dappweb/kolmarket/tree/main/arc-hackathon

Website:
https://kolmarket.ai

We are looking for Arc builder feedback, ecosystem review guidance, and the right path for official Arc/Circle review.
```

Do not publish until the user confirms at action time.

## Priority 3 - Arc Discord

First message:

```text
Hi Arc builders, KOLMarket is building USDC-native creator growth markets and campaign settlement infrastructure on Arc. We have prepared a public Arc review package here: https://github.com/dappweb/kolmarket/tree/main/arc-hackathon

Is there a recommended channel or form for ecosystem review / builder spotlight consideration?
```

Do not send until the user confirms at action time and channel rules are visible.

## Priority 4 - Arc X Follow-up

Short reply:

```text
KOLMarket is building USDC-native creator growth markets and campaign settlement infrastructure on Arc.

Public Arc review package:
https://github.com/dappweb/kolmarket/tree/main/arc-hackathon

Would appreciate Arc builder feedback and the right path for ecosystem review.
```

Use only on relevant Arc builder, hackathon, or ecosystem posts. Avoid mass-tagging affiliates.

## Priority 5 - Circle Grants / Questbook

Use when Questbook exposes an application form.

One-line summary:

```text
KOLMarket is building USDC-native creator growth markets and campaign settlement infrastructure on Arc, using stablecoin-native campaign budgets, KOL attribution, Growth Pool reward state, and a path toward Circle/CCTP-aligned settlement workflows.
```

Review package:

```text
https://github.com/dappweb/kolmarket/tree/main/arc-hackathon
```

Use `public/arc/circle-grants-application-draft.md` for full answers.

Do not submit until the user confirms at action time.

## After Each External Action

Update:

- `public/arc/arc-submission-manifest.json`
- `public/arc/arc-evidence-pack-template.md`
- `public/arc/arc-follow-up-log.md`
- `public/arc/chrome-arc-handoff.md`

Only set official flags to true with direct official-channel evidence.
